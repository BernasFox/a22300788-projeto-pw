...
def index_view(request):
    global_id = 1110600 # ID para Lisboa
    ...
    for day in data['data']:
        if(day["idWeatherType"] >= 10):
            day["idWeatherType"] = "imagens/w_ic_d_" + str(day["idWeatherType"]) + ".svg"
        else:
            day["idWeatherType"] = "imagens/w_ic_d_0" + str(day["idWeatherType"]) + ".svg"
...
def city_dia_view(request, id, dia):
    ...
    if response.status_code == 200:
        ...
        # Preparando a lista de localidades
        localidades_lista = [
            {"id": localidade['globalIdLocal'], "nome": localidade['local']}
            for localidade in localidades
        ]
    ...
    if(data['data'][dia]["idWeatherType"] >= 10):
        data['data'][dia]["idWeatherType"] = "imagens/w_ic_d_" + str(data['data'][dia]["idWeatherType"]) + ".svg"
    else:
        data['data'][dia]["idWeatherType"] = "imagens/w_ic_d_0" + str(data['data'][dia]["idWeatherType"]) + ".svg"
